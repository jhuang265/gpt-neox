    ...
    if args.test and torch.distributed.get_world_size() == 1:
        ...
        with torch.no_grad():
            ...
            for i in range(6):
                print("Layer ",i)
                print()
                hf = hf_model.gpt_neox.layers[i].attention.activations
                neox = model.module.sequential[2+i].attention.activations

                b_neox = model.module.sequential[2+i].attention.dense.bias
                w_neox = model.module.sequential[2+i].attention.dense.weight

                b_hf = hf_model.gpt_neox.layers[i].attention.dense.bias
                w_hf = hf_model.gpt_neox.layers[i].attention.dense.weight


                print('b_neox',b_neox.dtype)
                print('w_neox',w_neox.dtype)
                print('b_hf',b_hf.dtype)
                print('w_hf',w_hf.dtype)


                print("bias diff",(b_neox-b_hf).sum())
                print("weight diff",(w_neox-w_hf).sum())


                
                o = F.linear(hf['before dense'].permute(1,0,2), w_hf) \
                    + b_hf

                print("real diff",(o - neox['after dense']).sum())


                
                o2 = F.linear(neox['before dense'], w_neox) \
                    + b_neox

                o22 = F.linear(neox['before dense'], w_neox, b_neox) 

                print("real diff neox",(o2 - neox['after dense']).sum())



                temp = torch.nn.Linear(*list(w_neox.shape))
                temp.weight = torch.nn.Parameter(w_neox)
                temp.bias = torch.nn.Parameter(b_neox)

                o3 = temp(neox['before dense'])
                print("real diff hf to neox Linear",(o3 - hf['after dense'].permute(1,0,2)).sum())
                print("real diff hf to neox",(o2 - hf['after dense'].permute(1,0,2)).sum())
                print("real diff hf to neox",(o22 - hf['after dense'].permute(1,0,2)).sum())
                print()
                for k,v in neox.items():
                    if k in ['before dense','after dense']:
                        # print(v.shape, hf[k].shape)
                        print(k,(hf[k].permute(1,0,2) - v).sum())
                    elif k in ['attn_scores','attention_probs after smx--']:
                        print(v.shape, hf[k].shape)
                        print(k,(hf[k] - v).sum())
                    elif k == 'smx input':
                        print()
                        print(v.shape, hf[k].shape)
                        print(k,(hf[k] - v).mean())
                        # w = torch.where(hf[k] != v)
                        # print('where neox',v[w][0,...])
                        # print('where hf',hf[k][w][0,...])

                        print()

                    elif k in ['attention_probs']:
                        # print(v.shape, hf[k].shape)
                        print(k,(hf[k].view(80,32,32) - v).sum())
                    elif k == 'value before final bmm':
                        # print(v.shape, hf[k].shape)
                        print(k,(hf[k].view(80,32,64).permute(1,0,2) - v).sum())

                    else:
                        # print(v.shape, hf[k].shape)
                        print(k,(hf[k].permute(2,0,1,3) - v).sum())
                ...
                hfo = hf_model.gpt_neox.layers[i].act
                neoxo = model.module.sequential[2+i].act

                # print(hf_model.gpt_neox.layers[i].mlp.act)
                # print(model.module.sequential[2+i].mlp.activation_type)
                # print(model.module.sequential[2+i].mlp.activation_func)
                # print(hf_model.config)

                for k,v in neoxo.items():
                    print(k,hfo[k].shape, v.shape)
                    print(k,(hfo[k].permute(1,0,2) - v).sum())

            # exit(0)

            # inv_freq_hf = hf_model.gpt_neox.layers[0].attention.rotary_emb.inv_freq
            # inv_freq_neox = model.module.sequential[2].attention.rotary_emb.inv_freq
            # print("emb diff",inv_freq_hf.device,inv_freq_hf.dtype,inv_freq_neox.device,inv_freq_neox.dtype)
            # print("inv_freq_diff",(inv_freq_hf - inv_freq_neox.to('cuda:0')).sum())

            # emb_hf = hf_model.gpt_neox.layers[0].attention.rotary_emb.emb
            # emb_neox = model.module.sequential[2].attention.rotary_emb.emb
            # print("emb diff",emb_hf.device,emb_hf.dtype,emb_neox.device,emb_neox.dtype)
            # print("emb diff",(emb_hf - emb_neox).sum())


            # f_hf = hf_model.gpt_neox.layers[0].attention.rotary_emb.freqs
            # f_neox = model.module.sequential[2].attention.rotary_emb.freqs
            # print("freq diff",f_hf.device,f_hf.dtype,f_neox.device,f_neox.dtype)
            # print("freq diff",(f_hf - f_neox).sum())

            # t_hf = hf_model.gpt_neox.layers[0].attention.rotary_emb.t
            # t_neox = model.module.sequential[2].attention.rotary_emb.t
            # print("t diff",t_hf.device,t_hf.dtype,t_neox.device,t_neox.dtype)
            # print("t diff",(t_hf - t_neox).sum())
